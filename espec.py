...
from _misc import load_dict_from_file, points_to_roi
from datalog import Logger
from backfilt import filterImage
from fitting import setup_double_2d_gauss_model, fit_double_gauss2d_lm
...
if points_file is not None:
    analyse = {
        k: points_to_roi(v, w=1288, h=964, x_pad=150, y_pad=100)
        for k, v in load_dict_from_file(points_file).items()
        if len(v) > 0
    }
...
analyse = {
    "D:\\Bunker C\\2022\\Dec 05\\Pointing Lanex\\Run005\\HE\\Run005-12052022164625-530.tiff": [
        [250, 450],
        [800, 750],
    ],
    "D:\\Bunker C\\2022\\Dec 05\\Pointing Lanex\\Run005\\HE\\Run005-12052022171508-2240.tiff": [
        [200, 450],
        [650, 750],
    ],
}
...
if plotfit:
    if (
        "y"
        not in input(
            "Potting initialised with Fitting ON \nthis is EXTREEMLY SLOW \nconfirm? Y/N: "
        ).lower()
    ):
        plotfit = False
...
for day in days:
    ...
    if day == "22-12-05":
        ...
        day_keys = [
            f for f in list(analyse.keys()) if daystr in f and analyse[f] is not None
        ]
    ...
    for i, file in enumerate(tqdm(day_keys)):
        ...
        saveplot = "{}\\{}_plot.png".format(
            exportDir, name.split("\\")[-1].split("/")[-1].split(".")[0]
        )
        ...
        if bin != 1:
            ...
            full_back = (
                full_back.reshape(int(h / bin), bin, int(w / bin), bin).sum(3).sum(1)
            )
        ...
        fig.suptitle(name)
        ax["im"].imshow(
            sub, vmin=0, vmax=sub.max()
        )  # , extent = (e_x[0],e_x[-1],sub.shape[1],0))
        ...
        ax["im"].set_title(
            "total charge (arb. units) = {:.1e}".format(sub.sum(1).sum(0))
        )
        ...
        histfig.suptitle(
            "Calibration and Histogram for {} with {} shots".format(day, len(day_keys))
        )
