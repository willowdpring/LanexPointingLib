...
from _misc import *
from backfilt import get_background, walkDir, filterImage
from perspective import TransformToLanexPlane, get_transform
########################################################################
...
def test_MP(g=35000):
    ...
    print(
        "A grey value of {} indicates {:.2f}PSL implying {} electrons at {:.2e} Coulombs".format(
            g, p, int(n_e), q
        )
    )
...
def adjust_MP_image(file, alt=True, crop=[[0, 0], [-1, -1]]):
    ...
    plate_g = np.array(Image.open(file), np.float32)[
        crop[0][0] : crop[0][1], crop[1][0] : crop[1][1]
    ]  # [0][1]:crop[1][1],crop[0][0]:crop[1][0]]
    if alt:
        plate_q = np.clip(
            np.multiply(n_e_MP(alt_psl(plate_g)), electron_charge), 0, np.inf
        )
    else:
    ...
    fig.suptitle(
        file.split("/")[-1].split("\\")[-1].split("-")[1].replace("_", " ")
        + " "
        + filetype
        + " "
        + alt_str
    ...
    im_q = ax[1].matshow(
        plate_q, norm=LogNorm(vmin=max(plate_q.min(), 1.0e-17), vmax=plate_q.max())
    )
...
def integrate_lanex(
    ...
    if save_each:
        if not os.path.exists(
            exportDir + "//individuals_{}".format(saveas)
        ):  # check if it exists
    ...
    for i in tqdm(range(tot)):
        ...
        if save_each:
            save_u16_to_tiff(
                ...
                processed.shape[::-1],
                exportDir
                + "//individuals_{}".format(saveas)
                + "//PROC_{}".format(image.split("\\")[-1]),
            )
    ...
    save_u16_to_tiff(
        np.uint16(total_signal),
        total_signal.shape[::-1],
        exportDir + "//{}.tiff".format(saveas),
    )
    return total_signal
...
def clean_image_plate(file, alt=True, crop=[[0, -1], [0, -1]], bkg_scale=1):
    im_dat = np.array(Image.open(file), dtype=np.float64)[
        crop[0][0] : crop[0][1], crop[1][0] : crop[1][1]
    ]
    im_dat = im_dat[:, ::-1]
    ...
    fig, ax = plt.subplot_mosaic(
        [["orig"], ["slope"], ["signal"]], figsize=(9, 12), height_ratios=[1, 0.5, 1]
    )
    ...
    origIm = ax["orig"].imshow(
        np.multiply(im_dat_n_e, electron_charge), interpolation="none"
    )
    ax["orig"].set_title("image plate (cropped) (C/px)")
    cax2 = fig.colorbar(origIm, ax=ax["orig"], pad=0, shrink=1, location="right")
    ...
    im_dat_cropped = np.multiply(
        np.clip(im_dat_subbed[1200:-1000, 500:7500], 0.01, np.inf), electron_charge
    )
    sigIm = ax["signal"].imshow(im_dat_cropped, norm=LogNorm(), interpolation="none")

    ax["signal"].set_title(
        f"total Signal (C /px) with background scaled by {bkg_scale}"
    )
    cax1 = fig.colorbar(sigIm, ax=ax["signal"], pad=0, shrink=1, location="right")
    fig.tight_layout()
    fig.show()

    peaks = peak_local_max(
        im_dat_cropped, num_peaks=10, threshold_rel=1e-16, min_distance=200
    )
    # print(peaks)
    ...
    for peak in peaks:
        ...
        integral = integrate_circular_patch(
            im_dat_cropped, (center_y, center_x), radius
        )
    ...
    cax1 = fig2.colorbar(fullIm, ax=ax2, pad=0.01, shrink=0.6, location="right")
...
def transform_image(image, transformation_matrix, output_shape):
    ...
    transformed_image = cv2.warpAffine(
        image, transformation_matrix, (output_width, output_height)
    )
...
def plot_overlay_images(image1, image2, tmax=200, Mmax=2):
    ...
    slider_translation_x = Slider(
        ax_translation_x, "Translation X", -tmax, tmax, valinit=translation_x
    )
    slider_translation_y = Slider(
        ax_translation_y, "Translation Y", -tmax, tmax, valinit=translation_y
    )
    slider_rotation = Slider(ax_rotation, "Rotation", -10, 10, valinit=rotation_deg)
    slider_scale = Slider(
        ax_scale, "Scale", 1 / Mmax, Mmax, valinit=scale
    ...
    textbox_translation_x = TextBox(
        plt.axes([0.75, 0.5, 0.08, 0.025]), "", initial=str(translation_x)
    )
    textbox_translation_x.on_submit(
        create_textbox_handler(slider_translation_x, "translation_x")
    )

    textbox_translation_y = TextBox(
        plt.axes([0.75, 0.45, 0.08, 0.025]), "", initial=str(translation_y)
    )
    textbox_translation_y.on_submit(
        create_textbox_handler(slider_translation_y, "translation_y")
    )

    textbox_rotation = TextBox(
        plt.axes([0.75, 0.4, 0.08, 0.025]), "", initial=str(rotation_deg)
    )
    ...
    def save_or_print(event):
        ...
        transformation_matrix = get_transform(
            image1, translation_x, translation_y, rotation_deg, scale, scale
        )
        print("=" * 50)
...
def overlay_images(
    ...
    transformed_image2 = transform_image(
        working_image, rotation_scale_matrix, image1.shape
    )
